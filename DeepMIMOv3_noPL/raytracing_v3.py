...
from DeepMIMOv3_noPL.utils import safe_print, PathVerifier, dbm2pow
import scipy.io
import glob
...
def extract_data_from_ray(ray_data, rx_locs, params):
    ...
    if num_channels>0:
        ...
        # 忽略 PathLoss：設置為 1
        data[c.OUT_PL] = np.ones(num_channels)
...
def load_variables(path_params, path_verifier, params):
    ...
    user_data[c.OUT_PATH_RX_POW] = np.ones(path_params[2, :num_max_paths])
