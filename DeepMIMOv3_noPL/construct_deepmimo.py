...
from DeepMIMOv3_noPL.ant_patterns import AntennaPattern
...
def generate_MIMO_channel(raydata, params, tx_ant_params, rx_ant_params):
    ...
    for i in tqdm(range(len(raydata)), desc='Generating channels'):
        ...
        if raydata[i][c.OUT_PATH_NUM]==0:
            ...
            continue
        # ===== 添加一行代码禁用 PathLoss =====
        raydata[i][c.OUT_PATH_RX_POW] = np.ones_like(raydata[i][c.OUT_PATH_RX_POW])  # 禁用 PathLoss
...
class OFDM_PathGenerator:
    ...
    def no_LPF(self, raydata, Ts):
        ...
        # ===== 添加一行代码禁用 PathLoss =====
        power = np.ones_like(power)  # 禁用 PathLoss
